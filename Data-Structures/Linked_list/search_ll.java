/**
 * This class represents an the node of the linked list;
 *
 */
class node {
    int data;
    node link;
}

/**
 * This class represents a linked list.
 */
class linkedlist {
    node head;

    /**
     * Initializes a new instance of the linkedlist class with the given element.
     *
     * @param value initially the value to be inserted
     */
    linkedlist(int value) {
        // initial construction with one value
        this.head = new node();
        head.data = value;
        head.link = null;
    }

    /**
     * Initializes a new instance of the linkedlist class.
     *
     */
    linkedlist() {
    }

    /**
     * Prints all the elements in the linked list.
     */
    void print() {
        node temp = head;
        while (temp != null) {
            System.out.printf("%d ", temp.data);
            temp = temp.link;
        }
        System.out.printf("\n");
    }

    /**
     * Inserts an element at the end of the linked list.
     *
     * @param value the value to be inserted
     */
    void addelement(int value) {

        // creating a new node
        node obj = new node();
        obj.data = value;
        obj.link = null;

        if (head == null) {
            this.head = new node();
            head = obj;
            return;
        }
        node temp = head;
        while (temp.link != null) {
            temp = temp.link;
        }
        temp.link = obj;
        return;
    }

    /**
     * search the linked list for the presence of the element
     *
     * @param value the value to be searched.
     */
    boolean linear_search(int value) {
        if (head == null) {
            System.out.println("No value in the linked list");
            return false;
        }
        node temp = head;
        while (temp != null) {
            if (temp.data == value)
                return true;
            temp = temp.link;
        }
        return false;
    }
}

class search_ll {
    public static void main(String[] args) {
        linkedlist list = new linkedlist();
        list.addelement(8);
        list.addelement(18);
        list.addelement(28);
        list.addelement(158);
        list.addelement(265);
        list.print();
        if (list.linear_search(158))
            System.out.println("The element is present in the Linked list");
        else
            System.out.println("the element is not present in the linked list");
    }
}